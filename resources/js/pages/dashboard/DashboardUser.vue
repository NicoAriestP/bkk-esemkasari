        ...
        <div class="space-y-6">
            <div class="rounded-lg bg-gradient-to-r from-blue-600 to-blue-800 p-6 text-white">
                ...
                <p class="text-blue-100">
                    Pantau pengumuman, kuisioner, siswa tracer study, dan mitra DU/DI dari satu dashboard.
                </p>
            </div>
                    ...
                    <template #content>
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-slate-400">{{ totalQuestionnaires }} total, {{
                                    expiredQuestionnaires }} sudah kedaluwarsa</p>
                            </div>
                ...
                <Card class="xl:col-span-3">
                    <template #title>
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                            ...
                            <Select :modelValue="selectedMonthlyRange" :options="monthlyRangeOptions"
            ...
            <div class="grid gap-6 grid-cols-1 lg:grid-cols-2">
                    ...
                    <template #content>
                        <div class="space-y-3">
                            <div v-for="announcement in recentAnnouncements" :key="announcement.id"
                                class="flex items-start justify-between rounded-xl bg-slate-50 p-3">
                                <div class="flex flex-1 min-w-0 gap-3">
                                    <Avatar icon="pi pi-megaphone" class="bg-sky-100 text-sky-600 flex-shrink-0"
                                        shape="circle" />
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="text-xs text-slate-400">{{ formatDateTime(announcement.created_at) }}
                                        </p>
                                    </div>
                                </div>
                                <Button icon="pi pi-pencil" rounded text severity="warn" size="small"
                                    class="flex-shrink-0"
                ...
                <Card>
                    ...
                    <template #content>
                        <div class="space-y-3">
                            <div v-for="questionnaire in recentQuestionnaires" :key="questionnaire.id"
                                class="flex items-start justify-between rounded-xl bg-slate-50 p-3">
                                <div class="flex min-w-0 flex-1 gap-3">
                                    <Avatar icon="pi pi-list-check"
                                        class="bg-emerald-100 text-emerald-600 flex-shrink-0" shape="circle" />
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="text-xs text-slate-400">{{ questionnaire.responses_count }} respons
                                        </p>
                                        <p v-if="questionnaire.due_at" class="text-xs text-slate-400">Tenggat: {{
                                            formatDate(questionnaire.due_at) }}</p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-2 flex-shrink-0">
                                    <Button icon="pi pi-list" rounded text severity="info" size="small"
                ...
                <Card>
                    ...
                    <template #content>
                        <div class="space-y-3">
                            <div v-for="student in recentTracerStudyStudents" :key="student.id"
                                class="flex items-start justify-between rounded-xl bg-slate-50 p-3">
                                <div class="flex min-w-0 flex-1 gap-3">
                                    <Avatar icon="pi pi-users" class="bg-blue-100 text-blue-600 flex-shrink-0"
                                        shape="circle" />
                                    <div class="min-w-0 flex-1">
                                        <p class="font-medium text-slate-900">{{ student.name }}</p>
                                        <p class="text-sm text-slate-500">{{ student.student_class?.name }} - {{
                                            student.student_class?.year?.year }}</p>
                ...
                <Card>
                    ...
                    <template #content>
                        <div class="space-y-3">
                            <div v-for="partner in recentPartners" :key="partner.id"
                                class="flex items-start justify-between rounded-xl bg-slate-50 p-3">
                                <div class="flex min-w-0 flex-1 gap-3">
                                    <Avatar icon="pi pi-briefcase" class="bg-rose-100 text-rose-600 flex-shrink-0"
                                        shape="circle" />
                                    <div class="min-w-0 flex-1">
