...
watch(() => activityDetailsData.value.mainActivity, (newValue) => {
    const labels = (props.tracerStudyOptions as any)?.activityMain?.labels || {};
    const activityLabel = labels[newValue as string];
    stepItems.value[1].label = activityLabel ? `Lulusan ${activityLabel}` : 'Detail Aktivitas';
}, { immediate: true });
...
const studentActivityMainOptions = ref((props.tracerStudyOptions as any)?.studentActivityMain?.options || []);
const workTypeOptions = ref((props.tracerStudyOptions as any)?.workType?.options || []);
const activityMainOptions = ref((props.tracerStudyOptions as any)?.activityMain?.options || []);
const workingOptions = ref({
    salaryOptions: (props.tracerStudyOptions as any)?.salary?.options || [],
    jobChangeOptions: (props.tracerStudyOptions as any)?.jobChange?.options || [],
    howFoundJobOptions: (props.tracerStudyOptions as any)?.howFoundJob?.options || [],
    jobRelevanceOptions: (props.tracerStudyOptions as any)?.jobRelevance?.options || [],
});
const universityOptions = ref({
    educationLevelOptions: (props.tracerStudyOptions as any)?.educationLevel?.options || [],
    fundingSourceOptions: (props.tracerStudyOptions as any)?.fundingSource?.options || [],
    majorRelevanceOptions: (props.tracerStudyOptions as any)?.majorRelevance?.options || [],
});
const entrepreneurOptions = ref({
    businessScaleOptions: (props.tracerStudyOptions as any)?.businessScale?.options || [],
    businessIncomeOptions: (props.tracerStudyOptions as any)?.businessIncome?.options || [],
});
const feedbackOptions = ref({
    smkReasonOptions: (props.tracerStudyOptions as any)?.smkReason?.options || [],
    pklDurationOptions: (props.tracerStudyOptions as any)?.pklDuration?.options || [],
    qualityRatingOptions: (props.tracerStudyOptions as any)?.pklQuality?.options || [],
    certificateOwnershipOptions: (props.tracerStudyOptions as any)?.certificateOwnership?.options || [],
});
</script>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 mb-8 overflow-hidden">
                    ...
                    <div class="p-6 space-y-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                <div
                                    v-for="option in studentActivityMainOptions"
                                    :key="`study-${option.key}`"
                                    class="flex items-center p-4 border border-gray-200 rounded-xl hover:border-blue-300 transition-colors duration-200"
                                    :class="{ 'border-blue-500 bg-blue-50': studentActivityData.isStudying === option.key }"
                                >
                                    <RadioButton v-model="studentActivityData.isStudying" :inputId="`study-${option.key}`" name="isStudying" :value="option.key" />
                                    <label :for="`study-${option.key}`" class="ml-3 text-gray-700 font-medium cursor-pointer">{{ option.text }}</label>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="space-y-4 p-4 bg-gray-50 rounded-xl">
                                ...
                                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                    <div
                                        v-for="option in studentActivityMainOptions"
                                        :key="`work-${option.key}`"
                                        class="flex items-center p-4 border border-gray-200 rounded-xl hover:border-green-300 transition-colors duration-200 bg-white"
                                        :class="{ 'border-green-500 bg-green-50': studentActivityData.isWorking === option.key }"
                                    >
                                        <RadioButton v-model="studentActivityData.isWorking" :inputId="`work-${option.key}`" name="isWorking" :value="option.key" />
                                        <label :for="`work-${option.key}`" class="ml-3 text-gray-700 font-medium cursor-pointer">{{ option.text }}</label>
                                    </div>
                                </div>
                            </div>
                        </template>
                        ...
                        <template v-if="studentActivityData.isStudying === 'Tidak' && studentActivityData.isWorking === 'Ya'">
                            <div class="space-y-4 p-4 bg-yellow-50 rounded-xl border border-yellow-200">
                                ...
                                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                    <div v-for="option in workTypeOptions" :key="option.key" class="flex items-center p-3 border border-yellow-200 rounded-lg hover:border-yellow-400 transition-colors duration-200 bg-white" :class="{ 'border-yellow-500 bg-yellow-50': studentActivityData.workType === option.key }">
                                        <RadioButton v-model="studentActivityData.workType" :inputId="option.key" name="workType" :value="option.key" />
                                        <label :for="option.key" class="ml-3 text-gray-700 font-medium cursor-pointer">{{ option.text }}</label>
                                    </div>
                                </div>
                            </div>
                        </template>
            ...
            <div v-if="activeStep === 1" class="space-y-8">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6">
                        <ActivityDetails
                            v-model="activityDetailsData"
                            :activity-options="activityMainOptions"
                            :working-options="workingOptions"
                            :university-options="universityOptions"
                            :entrepreneur-options="entrepreneurOptions"
                        />
                    </div>
                </div>
            </div>
            ...
            <div v-if="activeStep === 2" class="space-y-8">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6">
                        <StudentFeedback v-model="feedbackData" :feedback-options="feedbackOptions" />
                    </div>
                </div>
            </div>
