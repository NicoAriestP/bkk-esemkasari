...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    ...
    ...(props.isNewRecord ? [] : [{ title: props.model.title, href: '#' }]),
    { title: props.isNewRecord ? 'Buat Kuisioner' : 'Edit Kuisioner', href: '#' },
]);
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-2xl font-bold text-gray-900">
                                {{ isNewRecord ? 'Buat Kuisioner Baru' : 'Edit Kuisioner' }}
                            </h1>
                            <p class="mt-1 text-sm text-gray-600">
                                {{ isNewRecord ? 'Isi form di bawah untuk membuat kuisioner baru' : 'Perbarui informasi kuisioner' }}
                            </p>
                        </div>
                        <Button label="Kembali" icon="pi pi-arrow-left" severity="secondary" outlined
                        ...
                        <template #content>
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <InputText id="title" v-model="form.title" placeholder="Masukkan judul kuesioner"
                                        class="w-full" :class="{ 'p-invalid': form.errors.title }" />
                                ...
                                <div>
                                    ...
                                    <Textarea id="description" v-model="form.description" rows="4"
                                        placeholder="Masukkan deskripsi kuesioner" class="w-full"
                    ...
                    <Card>
                        <template #title>
                            <div class="flex items-center justify-between">
                                ...
                                <Button label="Tambah Pertanyaan" icon="pi pi-plus" size="small" @click="addQuestion"
                        ...
                        <template #content>
                            <div v-if="form.questions.length === 0" class="py-8 text-center">
                                ...
                                <p class="text-gray-500">Belum ada pertanyaan. Klik tombol "Tambah Pertanyaan" untuk
                                    memulai.</p>
                            </div>

                            <div v-else class="space-y-4">
                                <TransitionGroup name="list">
                                    <Card v-for="(question, qIndex) in form.questions" :key="qIndex" class="border-l-4"
                                        :class="{
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <div class="flex items-start justify-between gap-4">
                                                    ...
                                                    <Button icon="pi pi-trash" severity="danger" text rounded
                                                        @click="removeQuestion(qIndex)" :disabled="form.processing"
                                                ...
                                                <div>
                                                    ...
                                                    <div class="grid grid-cols-2 gap-3 md:grid-cols-4">
                                                        <div v-for="typeOption in questionTypes" :key="typeOption.value"
                                                            ...
                                                            <div class="text-center">
                                                                ...
                                                                <p class="text-sm font-medium"
                                                                    :class="question.type === typeOption.value ? 'text-blue-900' : 'text-gray-700'">
                                                                    {{ typeOption.label }}
                                                                </p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <small v-if="(form.errors as any)[`questions.${qIndex}.type`]"
                                                        class="text-red-500">
                                                ...
                                                <div>
                                                    ...
                                                    <Textarea v-model="question.question_title" rows="3"
                                                        placeholder="Masukkan pertanyaan" class="w-full"
                                                        :class="{ 'p-invalid': (form.errors as any)[`questions.${qIndex}.question_title`] }" />
                                                    <small
                                                        v-if="(form.errors as any)[`questions.${qIndex}.question_title`]"
                                                        class="text-red-500">
                                                ...
                                                <div>
                                                    <label
                                                        class="mb-2 block font-medium text-gray-700">Catatan/Petunjuk</label>
                                                    <InputText v-model="question.notes"
                                                        placeholder="Tambahkan catatan atau petunjuk (opsional)"
                                                        class="w-full" />
                                                    <small class="text-gray-500">Contoh: "Pilih salah satu jawaban yang
                                                        sesuai"</small>
                                                </div>

                                                <!-- Opsi Jawaban (untuk dropdown & checkbox) -->
                                                <div
                                                    v-if="question.type === 'dropdown' || question.type === 'checkbox'">
                                                    <div class="mb-2 flex items-center justify-between">
                                                        <label class="font-medium text-gray-700">
                                                            Opsi Jawaban <span class="text-red-500">*</span>
                                                        </label>
                                                        <Button label="Tambah Opsi" icon="pi pi-plus" size="small" text
                                                            @click="addOption(qIndex)" :disabled="form.processing" />
                                                    </div>

                                                    <div v-if="!question.options || question.options.length === 0"
                                                        class="rounded-lg border-2 border-dashed border-gray-300 p-4 text-center">
                                                        <p class="text-sm text-gray-500">Belum ada opsi. Klik "Tambah
                                                            Opsi" untuk menambahkan.</p>
                                                    </div>

                                                    <div v-else class="space-y-2">
                                                        <TransitionGroup name="list">
                                                            <div v-for="(option, oIndex) in question.options"
                                                                :key="oIndex" class="flex items-center gap-2">
                                                                <i :class="question.type === 'checkbox' ? 'pi pi-check-square' : 'pi pi-circle'"
                                                                    class="text-gray-400"></i>
                                                                <InputText v-model="option.option_label"
                                                                    :placeholder="`Opsi ${oIndex + 1}`" class="flex-1"
                                                                ...
                                                                <Button icon="pi pi-times" severity="danger" text
                                                                    rounded @click="removeOption(qIndex, oIndex)"
                                                                    :disabled="form.processing"
                                                                    v-tooltip.left="'Hapus Opsi'" />
                                                            </div>
                                                        </TransitionGroup>
                                                    </div>
                                                    <small v-if="(form.errors as any)[`questions.${qIndex}.options`]"
                                                        class="text-red-500">
                                                ...
                                                <div v-if="question.type" class="rounded-lg bg-gray-50 p-4">
                                                    ...
                                                    <div v-else-if="question.type === 'checkbox' && question.options && question.options.length > 0"
                                                        class="space-y-2">
                                                        <div v-for="(option, idx) in question.options.slice(0, 3)"
                                                            :key="idx" class="flex items-center gap-2">
                                                            <Checkbox :inputId="`preview-${qIndex}-${idx}`" disabled />
                                                            <label :for="`preview-${qIndex}-${idx}`"
                                                                class="text-sm text-gray-600">{{ option.option_label
                                                                }}</label>
                                                        </div>
                                                        <p v-if="question.options.length > 3"
                                                            class="text-xs text-gray-500">+{{ question.options.length -
                                                            3 }} opsi lainnya</p>
                                                    </div>
                                                    <div v-else-if="question.type === 'fillable'">
                                                        <Textarea rows="3"
                                                            placeholder="Jawaban akan diisi oleh responden"
                                                    ...
                                                    <div v-else>
                                                        <p class="text-sm text-gray-500">Pilih tipe pertanyaan terlebih
                                                            dahulu</p>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex gap-2">
                                    <Button label="Batal" severity="secondary" outlined
                                        @click="router.visit(route('questionnaires.index'))"
                                        :disabled="form.processing" />
                                    <Button :label="isNewRecord ? 'Simpan Kuesioner' : 'Perbarui Kuesioner'"
